...
export default function ResetPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post(`/api/auth/reset-password/${encodeURIComponent(token)}`, {
        newPassword,
        confirmPassword
      });
    ...
    <div className="login-vanta-shell fixed inset-0 top-16 overflow-y-auto px-3 py-3 sm:overflow-hidden sm:px-6 sm:py-4 lg:py-6">
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100svh-5rem)] max-w-5xl items-center gap-5 sm:h-[calc(100vh-5rem)] lg:-translate-y-4 lg:grid-cols-[1fr_420px] lg:gap-8">
        <section className="hidden lg:block">
          ...
          <h1 className="login-hero-title mt-3 max-w-xl text-4xl font-bold tracking-normal">Create a fresh password and continue tracking.</h1>
          <p className="login-creator-credit mt-4 text-sm font-semibold">Use a password you do not use elsewhere.</p>
        </section>
        ...
        <div className="login-card card mx-auto w-full max-w-md">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="muted mt-1">Choose a new password for your account.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700 dark:text-slate-200">New password</label>
              <input
                type="password"
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700 dark:text-slate-200">Confirm password</label>
              <input
                type="password"
                className="input-field"
          ...
          <p className="mt-5 text-center text-sm text-slate-500 dark:text-slate-400">
            Need a new link? <Link to="/forgot-password" className="text-primary-600 hover:underline">Request reset</Link>
          </p>
        </div>
      </div>
    </div>
